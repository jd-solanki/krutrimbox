...
krutrimbox uses a custom Docker Sandboxes template image for Target Issue Sandboxes instead of the stock `docker/sandbox-templates:codex` image.
...
The custom image is built from `Dockerfile.sandbox`, extends `docker/sandbox-templates:codex`, and installs `pnpm@10.23.0`. The factory creates Target Issue Sandboxes with `--template docker.io/library/krutrimbox-codex:pnpm` by default. Operators may override that image with `KRUTRIMBOX_SANDBOX_TEMPLATE` when testing another template.
