...
Sandboxed Codex sessions run through `codex exec --ephemeral --ask-for-approval never --sandbox danger-full-access`. The no-approval setting is required because Code Factory runs Codex non-interactively; an approval prompt would otherwise hang or fail the AFK Issue. `danger-full-access` is scoped by the outer Docker Sandbox clone, not by the host machine, and avoids Codex's inner sandbox blocking package installs, tests, read-only GitHub inspection, or other commands the Sandboxed Agent may need.
